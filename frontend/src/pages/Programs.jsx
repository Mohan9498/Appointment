  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-black dark:text-white">
      ...
      <section className="py-10 md:py-28 relative overflow-hidden">
        ...
        {/* HEADER */}
        <div className="text-center mb-8 md:mb-16 px-4 relative">
          <span className="inline-block px-3 py-1 md:px-4 md:py-1.5 rounded-full text-xs font-semibold tracking-wider uppercase bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 mb-3 md:mb-6 animate-fade-in-up">
            Programs
          </span>
          <h2 className="text-3xl md:text-6xl font-extrabold tracking-tight text-gray-900 dark:text-white animate-fade-in-up-delay-1">
            Our <span className="text-gradient">Programs</span>
          </h2>
          <p className="mt-2 md:mt-4 text-gray-500 dark:text-gray-400 max-w-xl mx-auto text-sm md:text-base leading-relaxed animate-fade-in-up-delay-2">
            Carefully designed therapy programs to support your child's growth and development.
          </p>
        </div>

        {/* CARDS */}
        <div className="grid md:grid-cols-3 gap-4 md:gap-8 px-4 md:px-6 max-w-7xl mx-auto relative">

          {programs.map((program, index) => (
            <div
              key={index}
              className="group relative rounded-xl md:rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-700 bg-white dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.06] hover:-translate-y-1"
            >
              ...
              <div className="relative h-40 md:h-64 overflow-hidden">
                ...
                {/* Title on Image */}
                <div className="absolute bottom-2.5 md:bottom-4 left-3 md:left-5">
                  <h3 className="text-base md:text-xl font-bold text-white drop-shadow-lg">{program.title}</h3>
                </div>
              </div>

              {/* CONTENT */}
              <div className="p-3 md:p-6">
                <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 leading-relaxed mb-3 md:mb-5">{program.description}</p>

                <button
                  onClick={() => navigate(`/programs/${program.title.toLowerCase().replace(/\s+/g, "-")}`)}
                  className="inline-flex items-center gap-2 px-4 py-2 md:px-5 md:py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full text-xs md:text-sm text-white font-semibold hover:shadow-glow-blue hover:scale-[1.03] transition-all duration-300"
                >
                  Learn More
